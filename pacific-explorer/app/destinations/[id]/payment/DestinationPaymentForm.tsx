...
interface DestinationPaymentFormProps {
  bookingId: number;
  amount: number;
  currency: string;
...
export default function DestinationPaymentForm({
  bookingId,
  amount,
  currency,
  destinationId
}: DestinationPaymentFormProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const { error, paymentIntent } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${window.location.origin}/destinations/${destinationId}/confirmation`,
        },
      ...
      if (paymentIntent && paymentIntent.status === 'succeeded') {
        ...
        router.push(`/destinations/${destinationId}/confirmation?payment_intent=${paymentIntent.id}&booking_id=${bookingId}`);
      } else {
